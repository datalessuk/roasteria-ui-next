...
interface CoffeeCardProps {
  coffee?: IRoastersCoffees;
}

export default function FeaturedCoffeeCard({ coffee }: CoffeeCardProps) {
  ...
  return (
    <div
      ...
      className="relative w-full aspect-[4/5] cursor-pointer rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-300 group"
    >
      <Image
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-b-xl">
        <h2 className="text-white font-bold text-lg truncate capitalize">
          {coffee?.name}
        </h2>
        {coffee?.roast_level && (
